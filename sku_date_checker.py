...
# --- THEME (safe fallback if missing) ---
try:
...
DEFAULT_CONFIG = {
    ...
    # New: worker pool size; tune per network/CPU
    "max_workers": "8",
    # New: CSV chunk size (rows) for streaming
...
# SQLite engine with WAL + sane pragmas for concurrency
...
class SkuCheckerApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        # HTTP session (keep-alive + retries)
    ...
    def _insert_result_into_treeview(self, result: SkuResult):
        ...
        # soft cap: keep last N rows to avoid UI degradation
    ...
    def _load_csv(self):
        ...
        if not path:
            return
        self.csv_path = path
        # Light preview: sample SKUs for test tab without loading entire CSV
        try:
        ...
        try:
            for chunk in pd.read_csv(
                ...
                for sku in zero:
                    ...
                    if sku not in seen:
                        seen.add(sku)
                        # bounded queue blocks (backpressure)
                        self.task_queue.put(sku)
                        self.total_enqueued += 1
                        # progress maximum can grow dynamically
    ...
    def _check_queue(self):
        ...
        try:
            ...
            # Continue pumping until done
            if self.is_processing:
                self.root.after(STATUS_UPDATE_MS, self._check_queue)
            else:
                return

            # Done condition: producer finished AND queues empty AND workers idled
    ...
    def _fetch_order_details(self, sku: str) -> dict:
        ...
        try:
            ...
            # manual guard loop on top of urllib3 retry to handle payload parsing/429 w/ Retry-After
